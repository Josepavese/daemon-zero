# DaemonZero Backup/Restore Backend Specification

## Overview
This specification defines the backend implementation for DaemonZero's backup and restore functionality, providing users with the ability to backup and restore their DaemonZero configurations, data, and custom files using glob pattern-based selection. The backup functionality is implemented as a dedicated "backup" tab in the settings interface for easy access and organization.
...
- Integrates with DaemonZero's established error handling and toast notification system
...
backup_section: SettingsSection = {
    ...
    "description": "Backup and restore DaemonZero data and configurations using glob pattern-based file selection.",
    "fields": [
        {
            "id": "backup_create",
            "title": "Create Backup",
...
def _get_default_patterns(self) -> str:
    ...
    return f"""# DaemonZero Knowledge (excluding defaults)
{agent_root}/knowledge/**
!{agent_root}/knowledge/default/**

# DaemonZero Instruments (excluding defaults)
...
/dz/knowledge/**
!/dz/knowledge/default/**
/root/**
!/root/.*/**
!/root/.*

# Host environment
/home/rafael/dz/data/knowledge/**
!/home/rafael/dz/data/knowledge/default/**
/home/rafael/**
!/home/rafael/.*/**
!/home/rafael/.*
```

> **⚠️ CRITICAL FILE NOTICE**: The `{agent_root}/.env` file contains essential configuration including API keys, model settings, and runtime parameters. This file is **REQUIRED** for DaemonZero to function properly and should always be included in backups alongside `settings.json`. Without this file, restored DaemonZero instances will not have access to configured language models or external services.
...
class BackupCreate(ApiHandler):
    ...
    async def process(self, input: dict, request: Request) -> dict | Response:
        patterns = input.get("patterns", "")
        include_hidden = input.get("include_hidden", False)
        backup_name = input.get("backup_name", "daemon-zero-backup")
...
The BackupService leverages DaemonZero's existing file operation helpers which already support RFC (Remote Function Call) routing for development mode. This ensures seamless operation whether running in direct mode or with container isolation.
...
class BackupService:
    """Core backup and restore service for DaemonZero"""

    def __init__(self):
        self.agent_zero_version = self._get_agent_zero_version()
        self.agent_zero_root = files.get_abs_path("")  # Resolved DaemonZero root
    ...
    def _get_agent_zero_version(self) -> str:
        """Get current DaemonZero version"""
        try:
    ...
    async def create_backup(self, patterns: str, include_hidden: bool = False, backup_name: str = "daemon-zero-backup") -> str:
    ...
    def create_backup_with_progress(self, patterns: str, include_hidden: bool = False, backup_name: str = "daemon-zero-backup"):
...
#### DaemonZero Internal Dependencies
The backup system requires these DaemonZero helper modules:
...
#### Integration with DaemonZero Error System
The backup system integrates with DaemonZero's existing error handling infrastructure:

```python
from python.helpers.errors import format_error
from python.helpers.print_style import PrintStyle

# Follow DaemonZero's error handling patterns
try:
...
- Restrict backups to predefined base directories (/dz, /root)
...
BACKUP_CONFIG = {
    ...
    "allowed_base_paths": ["/dz", "/root"]
}
```

#### Future Integration Opportunities
**Task Scheduler Integration:**
DaemonZero's existing task scheduler could be extended to support automated backups:
...
The backup system uses the same version detection method as DaemonZero's main UI:
...
```json
{
  ...
  "include_patterns": [
    ...
    "/dz/memory/**",
    "/dz/.env",
    "/dz/tmp/settings.json"
  ],
  "exclude_patterns": [
    "/dz/knowledge/default/**",
    "/dz/instruments/default/**",
    "/dz/memory/embeddings/**"
  ],
...
- **Environment Details**: User, timezone, working directory, runtime mode, DaemonZero root path
...
- **Compatibility Data**: DaemonZero version and environment for restoration validation
...
This enhanced backend specification provides a production-ready, comprehensive backup and restore system with advanced metadata tracking, real-time progress monitoring, and intelligent file management capabilities, all while maintaining DaemonZero's architectural patterns and security standards.
...
- **Testing Verified**: BackupService initializes correctly and detects DaemonZero root paths
...
**Problem:** When `include_hidden=false`, the system was excluding ALL hidden files, even when they were explicitly specified in patterns like `/dz/.env`.

**Solution:** Implemented explicit pattern detection that distinguishes between:
- **Explicit patterns** (like `/dz/.env`) - Always included regardless of `include_hidden` setting
- **Wildcard discoveries** (like `/dz/*`) - Respect the `include_hidden` setting

**Result:** Critical files like `.env` are now properly backed up when explicitly specified, ensuring DaemonZero configurations are preserved.
...
The DaemonZero backup system is now production-ready and battle-tested! 🚀
...
- Works seamlessly between different DaemonZero installations
...
The DaemonZero backup system now provides:
