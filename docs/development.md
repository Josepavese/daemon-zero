# Development manual for DaemonZero
This guide will show you how to setup a local development environment for DaemonZero in a VS Code compatible IDE, including proper debugger.
...
> [!NOTE]
> - DaemonZero runs in a Docker container, this simplifies installation and ensures unified environment and behavior across systems.
...
- DaemonZero is available on GitHub [github.com/daemon0ai/daemon-zero](https://github.com/daemon0ai/daemon-zero).
- You can download the files using a browser and extract or run `git clone https://github.com/daemon0ai/daemon-zero` in your desired directory.

> [!NOTE]
> In my case, I used `cd ~/Desktop` and `git clone https://github.com/daemon0ai/daemon-zero`, so my project folder is `~/Desktop/daemon-zero`.
...
- DaemonZero comes with `.vscode` folder that contains basic setup, recommended extensions, and debugger profiles. These will help us a lot.

1. Open your IDE and open the project folder using `File > Open Folder` and select your folder, in my case `~/Desktop/daemon-zero`.
...
## Step 4: Run DaemonZero in the IDE
Great work! Now you should be able to run DaemonZero from your IDE including real-time debugging.
...
After inserting my API key in settings, my DaemonZero instance works. I can send a simple message and get a response.
...
## Step 5: Run another instance of DaemonZero in Docker
- Some parts of DZ require standardized linux environment, additional web services and preinstalled binaries that would be unneccessarily complex to set up in a local environment.
- To make development easier, we can use existing DZ instance in docker and forward some requests to be executed there using SSH and RFC (Remote Function Call).

1. Pull the docker image `daemon0ai/daemon-zero` from Docker Hub and run it with a web port (`80`) mapped and SSH port (`22`) mapped.
If you want, you can also map the `/dz` folder to our local project folder as well, this way we can update our local instance and the docker instance at the same time.
This is how it looks in my example: port `80` is mapped to `8880` on the host and `22` to `8822`, `/dz` folder mapped to `/Users/frdel/Desktop/daemon-zero`:
...
You have successfully set up a complete DaemonZero development environment! You now have:
...
- ✅ The ability to develop, debug, and test DaemonZero features seamlessly

You're now ready to contribute to DaemonZero, create custom extensions, or modify the framework to suit your needs. Happy coding! 💻✨
...
- Navigate to your project root in the terminal and run `docker build -f DockerfileLocal -t daemon-zero-local --build-arg CACHE_DATE=$(date +%Y-%m-%d:%H:%M:%S) .`
